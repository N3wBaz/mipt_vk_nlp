...
class BpeTokenizer(ByteTokenizer):
    ...
    def train(self, texts: List[str], max_vocab: int) -> None:
        ...
        for _ in progress_bar:
            ...
            cnt = count_pairs(list_of_ids)  # Используем списки идентификаторов
    ...
    def encode(self, text: str) -> List[int]:
        ...
        # Последовательно применяем таблицу склеиваний в том порядке, в котором добавлялись токены в словаре
        while len(ids) > 1:
            cnt = count_pairs([ids])  # Передаем текущие идентификаторы как список
            pair = max(cnt, key=cnt.get)
